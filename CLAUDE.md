...
**Per-host tiers (`controller.behaviorKey()`), orthogonal to `injectionMode`:** `off` (denylisted or disabled — never runs) · `safe` (host in `settings.safeModeSites`) · `normal` (the `injectionMode` strategies above). **Safe mode** is for sites whose *continuous* DOM-integrity monitors catch even the ephemeral focus-write (confirmed: chatgpt.com bounces the instant a single transient `aria-label` lands). In safe mode the extension classifies but **never writes to the page** (no aria, no debug badge); the focus-target registry is reused, but the injector is armed `'speak'` instead of `'write'` — the user focuses an icon control and presses **Alt+Shift+I** (`SAFE_MODE_HOTKEY_LABEL`) to hear its label via `window.speechSynthesis` (a second voice over the screen reader — a degraded fallback, since in-context SR announcement *requires* a DOM write, which is exactly what these sites block). Labels also show in the popup readout. `isHostInList()` (denylist.ts) matches the list; changing any tier (`behaviorKey` transition) tears down + `resetHandled()` + rebuilds.

**Off-page readout (`GET_READOUT` → popup).** The controller keeps a per-icon readout (deduped by hash, capped) — thumbnail (`data:image/svg+xml` of the canonical SVG, or the `<img>` src) + label + confidence + state — rendered as `<img>` in the popup. SVG-in-`<img>` can't run script, so page-derived markup is safe in the privileged popup. Zero page contact → the safe way to verify what the model sees on protected sites (unlike the debug badge, which appends nodes to the page and trips checkpoints).

**No DOM sentinel.** "Have we processed this node" state lives in a `WeakSet` (`src/content/handled.ts`), NOT a `data-icon-labeler` attribute — we leave no foreign attribute for integrity monitors to hash. `SENTINEL_ATTR` survives only as a defensive entry in `extract.ts`'s volatile-attribute stripper. `resetHandled()` clears the set on a tier/mode switch.
